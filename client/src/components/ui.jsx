...
export function Btn({ to, onClick, type = 'button', variant = 'primary', children, className = '', disabled }) {
  const cls = `inline-flex items-center justify-center px-4 py-2 rounded-lg text-sm font-semibold transition disabled:opacity-50 disabled:cursor-not-allowed ${variants[variant]} ${className}`;
...
export function Alert({ kind = 'info', children }) {
  ...
  return <div className={`rounded-lg px-4 py-3 text-sm ${styles[kind]}`}>{children}</div>;
}
