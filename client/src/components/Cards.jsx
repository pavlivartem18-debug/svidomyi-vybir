  ...
  return (
    <article className="group hover-lift shadow-soft flex h-full flex-col overflow-hidden rounded-2xl bg-white dark:bg-slate-800">
      ...
      <div className="flex flex-1 flex-col p-5">
        <h3 className="font-bold leading-snug text-slate-900 dark:text-white">{news.title}</h3>
        ...
        <div className="mt-4 flex items-center justify-between text-xs text-slate-400">
          ...
          <Link to={`/news/${news.id}`} className="font-bold text-blue-600 transition hover:gap-2 hover:underline dark:text-blue-400">
            {useLang().t('readMore')} →
          </Link>
        </div>
      </div>
  ...
  return (
    <article className="group hover-lift shadow-soft flex h-full gap-4 rounded-2xl bg-white p-5 dark:bg-slate-800">
      ...
      <div className="min-w-0 flex-1">
        ...
        <h3 className="mt-1.5 font-bold leading-snug text-slate-900 dark:text-white">{event.title}</h3>
        ...
        <Link to={`/events/${event.id}`} className="mt-3 inline-block text-sm font-bold text-blue-600 hover:underline dark:text-blue-400">
          {t('details')} →
        </Link>
      </div>
    </article>
